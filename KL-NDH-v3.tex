...
distribution is given by the vector $\mu=\{\frac {k_1} {2E}, \frac {k_2} {2E},
\ldots , \frac {k_n} {2E}$. It ought to be noted that the stationary
...
distribution holds the local property that it does not change if weights in non
neighbors nodes are changes while keeping the total weight constant. 
The entropy rate represents the rate at which the entropy of a sequence e.g., a
random walk grow with n. Formally for a stochastic prcess $\{X_i\}$, the
entropy rate is given by
\begin{equation}
H(\mathcal{X})= \lim_{n \to \inf}\frac{1}{n} H(X_1,X_2, \ldots , X_n)
\end{equation}
For a stationary Markov chain, the entropy reate is  
\begin{equation}
...
We define the Markov processes for both the young subjects and the elder ones.
...
\end{array}
\end{equation}
where $T_{ij}^{y}$ and  $T_{ij}^{e}$ are the transition matrix of the young and
the elder subjects.
...
We compare the two measures of robustness, one based on efficiency \ref{} and
two based on robustness \ref{ay_geometric_2007} .One defines robustness of node
v in terms of loss of efficiency in the informational flow between the original netwotk and the
resulting network, te loss is obviously related to the node. Two, on the other
hand calcilates the robustness of node v relative to the removal of a node or
group of nodes. We find that 
