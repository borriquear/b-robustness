...
%OR invoke function degrees(adj)
degree2=degrees(RZcor);
degree=degree2;
%note that degree2=degree +1

%Study if connectivity degree is heavy tailed (power law) p(x)= x^-alpha
...
%diameter: The characteristic path length is the average shortest path length in the network. The global efficiency is the average inverse shortest path length in the network. The node eccentricity is the maximal shortest path length between a node and any other node. The radius is the minimum eccentricity and the diameter is the maximum eccentricity.
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%
 %Study Small World properties
 %Create a pool of randomized matrixz in order to see whether Cp and Lp are
 %undr or over represented.
 RZnumnodes=numnodes(RZcor);
 RZnumedges=numedges(RZcor);
 RRandcor=random_graph(numnodes(RZcor),[ ],numedges(RZcor))
 %create array of matrixes, each for a synthetic random matrix generated by
 %Tenyi Erdos alg.
 randomgraphs=zeros(90,90,90)
 M=100% number of random matrix to be generated
 for i= 1:M
     randomgraphs(i,:,:)=random_graph(RZnumnodes,[ ],RZnumedges)
 end
 % check if two synthetic matrizex are identical: 0 different.
 isequal(randomgraphs(4,:,:),randomgraphs(90,:,:))
 
 %%%
 % Sergei Maslov randomization algorithm
 sym_generate_srand(RZcor)
 
 
 % Measures Related with distance
 %% Efficiency and Vulnerability
 %convert RZcor in sparse matrix
 ...
 %mas rapido por lineas ...        
%graphshortestpath(spRZcor,90,'directed',false);

%Calculate Network vulnerability
%glob_n_vul=max(node_vul)

spRredu=sparse(RZcor);
node_eff=zeros(1,90);
    node_vul=zeros(1,90);
for i=1:90
    %buld new matrix deleting the node and all its links
    ...
    node_vul(1,i)= (glob_n_effic-node_eff(1,i))/glob_n_effic;
    spRredu=sparse(RZcor);
end
    % which index and value are the highrst value of vulnerability 
    %Vulnerability =0 , means RESILIENT, vulnerability =1 netw vulnerable
    [g_vulind,glob_n_vul]=max(node_vul)
    %ranking of vulnerability nodes
    [B,IX]=sort(node_vul)
    
%% Study of Clustering (triangles) and cycles 
