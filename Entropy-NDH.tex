...
\abstract{Network theory approaches to brain connectivity in resting state-
fMRI have been mainly focused on the study of topological properties and network motifs that
characterize the network structure. The network
...
interplay between brain disease and network connectivity is still missing. Here
we combine graph and information theory based approaches to understand
network robustness in resting state-fMRI.
We calculate first how the network
robustness is affected upon the removal of any node in the functional
connectivity network in resting state for both young and elder subjects.
Then, we study the stochastic process defined by a random walk on the functional
connectivity graph, to provide information theoretic measures such as the
entropy rate of the stationary Markov chain, defined by the transformation of
the binary adjacency matrix $A$ into a probability transition matrix $T$,
$T_{ij}= \frac {A_{ij}} {\sum_{j} {A_{ij}}}$. We find that the entropy rate of
the functional connectivity network modeled as a Markov chain explains why the removal of some nodes increases the
efficiency of the informational flow shown in the graph based approach.
We argue that the discovery of network based biomarkers for
neurodegenerative conditions will rely on the combination of both graph
...
the removal of any node in the functional connectivity network in resting state for both young and elder subjects. 
We provide a ranking of nodes that quantifies the impact of their obliteration
using a network efficiency measure based on \cite{latora_efficient_2001} that
quantifies how the network efficiency in transmitting information deteriorates
once a node is removed from the network.
...
in the graph based approach.This is explained in the results section
\ref{results}. the paper clonclude with a discussion section \ref{discussion}
...
provided in the Apendix \ref{appendix}.
...
high level of functional correlation during rest. Graph theory provides a
...
-default mode network, primary sensory motor network etc.- to the assessment of
...
Thus, the identification of different subnetworks during rest can be seen as a
 ...
 analysis (ICA) or clustering. Graph theory-based approaches model the brain as
 ...
 quantitatively characterize the overall topological organzation.   
 %adv
Graph-based techniques have proliferated in the last years providing new
insights into the structure funtion relantionship in the healthy brain, and
in aging and neuropathological disorders. Prove of the utility of this approach 
is that notable proponents of a modularist vision of brain connectivity to
...
brain networks approach to understand the interplay beween structure and
...
Thus, for a given network  $G(V,E)$ with an adjacency matrix $A$, a perturbation
$\delta$ that transforms $A$ into a new adjacency matrix $A'$ by deleting a
set of nodes $W$ from $V$. Thus, the initial graph $G(V,E)$ is transformed
into a new post perturbation graph $G(V-W,E-E(W))$, where $E-E(W)$ is the set
of edges that do not connect any of the deleted nodes in $W$
We want to
measure the network vulnerability, to do so we calculate the geodesic path distance  between any two nodes in the graph.
...
distances between pairs of nodes are smaller.The efficiency of the graph
$G(V,E)$, $\Gamma(G(V,E))$ is
\begin{equation}
\Gamma(G)=\frac{1}{n(n-1) \sum_{i \neq j \in V} \frac{1}{d_{ij}} }
\label{eq:latm}
\end{equation}
where $n$ is the number of nodes or $|V|$ and $d{ij}$ is the
...
that $0 \leq d_{ij}= d_{ji} \leq 1 $.
 ...
 measure \ref{eq:latm} in order to investigate the network functionality when
 a set of nodes are obliterated.
The efficiency $\Gamma(G)$
...
expression:
\begin{equation}
Vul(i)=\frac {\Gamma(G(V,E)) - \Gamma(G(V-i,E)) } \Gamma(G(V,E)) 
\label{eq:vul}
\end{equation}
...
in the overall network efficiency of a 22.7\% according to formula \ref{eq:vul}.
...
RELATE RESULTS  with the litterature on RSNs . What can we say about
...
of a stationay Markov chain which describes how a particle walks randomly from
...
random walk on a graph in which a node or grop of nodes has been removed is calcultated. 
